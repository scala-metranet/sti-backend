...
class ScoringService {
  public async getMaster(param: any): Promise<any> {
    ...
    let data: any = ModelScoringMaster.query()
      .select()
  ...
  public async get(param: any): Promise<any> {
    // const page = param.page - 1;
    let data: any = ModelScoringQuestion.query()
      .select()
  ...
  public async getHistory(param: any): Promise<any> {
    const page = param.page - 1;
    let data: any = ModelScoring.query()
      .select()
      .from(ModelScoring.tableName)
      .withGraphFetched(
        "[sprint,created(filterUser).[role,user_internship(filterUserIntern).internship],user(filterUser).[squad,role,user_internship(filterUserIntern).internship]]"
      )
      .whereNotDeleted()
      .modifiers({
        filterUser(builder) {
  ...
  public async getScore(param: any): Promise<any> {
    let data: any = ModelSprint.query()
      .select(
        "sprint.id",
        "sprint.sprint",
        "sprint.objective",
    ...
    if (param.user.role.name === "Mentor") {
      data = data
        .withGraphJoined(
          "[squad_leader(filterSquadLeader).user_internship(filterUserInternship).internship(filterInternship),squad(filterSquad).mentee(filterMentee).user_internship(filterIntern).internship(filterInternship)]"
        )
        .modifiers({
      ...
      data = data
        .withGraphJoined(
          "[squad_leader(filterSquadLeader).user_internship(filterUserInternship).internship(filterInternship),squad(filterSquad).mentor(filterMentee)]"
        )
        .modifiers({
          filterSquadLeader(builder) {
            builder.select("id", "name");
    ...
    if (param.user.role.name === "Mentor") {
      ...
      data.map((e) => {
        e.squad.mentee.map((men) => {
          men.score =
            datascore[e.id] != undefined
              ? datascore[e.id][men.id] != undefined
                ? datascore[e.id][men.id]
                : null
              : null;
        });
      });
  ...
  public async detailMaster(id: any): Promise<any> {
    const data: any = await ModelScoringMaster.query()
      .select()
      .withGraphFetched(
        "[company,role,internship_program,section.[question.[options]]]"
      )
      .from(ModelScoringMaster.tableName)
      .whereNotDeleted()
      .where("id", id)
  ...
  public async scoreForm(id: any, param: any): Promise<any> {
    const user: any = await ModelUser.query()
    ...
    if (user.role.name === "Mentor") {
      ...
      let user_internship: any = await ModelUserInternship.query()
        .withGraphFetched("[internship]")
        ...
        .first();
      if (!user_internship) {
        user_internship = await ModelUserInternship.query()
          .withGraphFetched("[internship]")
          .where("mentee_id", param.user.id)
    ...
    const data: any = await ModelScoringMaster.query()
      .select()
      .withGraphFetched(
        "[company, role, internship_program, scoring(scoringByUser).[detail], section.[question.[options]]]"
      )
      .modifiers({
        scoringByUser: (builder) => builder.where("user_id", id),
      })
      .from(ModelScoringMaster.tableName)
      .whereNotDeleted()
        ...
        section.question.map((question) => {
          for (var i = 0; i < keyReplacement.length; i++) {
            question.question = question.question.replace(
              new RegExp("{" + keyReplacement[i] + "}", "gi"),
              valueReplacement[i]
            );
          }
        });
  ...
  public async detail(id: any): Promise<any> {
    const data: any = await ModelScoringQuestion.query()
      .select()
      .from(ModelScoringQuestion.tableName)
      .whereNotDeleted()
      .where("id", id)
  ...
  public async detailScore(id: any): Promise<any> {
    const datafirst: any = await ModelScoring.query()
      .from(ModelScoring.tableName)
      .whereNotDeleted()
      .where("id", id)
      .first();
    ...
    const data: any = await ModelScoring.query()
      .select()
      .from(ModelScoring.tableName)
      .whereNotDeleted()
    ...
    const user_internship: any = await ModelUserInternship.query()
      .withGraphFetched("[mentee,internship]")
  ...
  public async create(param: any): Promise<any> {
    //create master
    const insertMaster: any = await ModelScoringMaster.query()
      .insert({
        id: generateId(),
        name: param.name,
    ...
    for (let index = 0; index < param.section.length; index++) {
      ...
      const insertSection: any = await ModelScoringSection.query()
        .insert({
          id: generateId(),
          name: element.name,
      ...
      for (
        let indexques = 0;
        indexques < element.questions.length;
        indexques++
      ) {
        const question = element.questions[indexques];
        const insertQuestion: any = await ModelScoringQuestion.query()
          .insert({
            id: generateId(),
        ...
        for (let indexopt = 0; indexopt < question.options.length; indexopt++) {
          ...
          const insertOption: any = await ModelScoringQuestionOption.query()
            .insert({
              id: generateId(),
              name: option.name,
  ...
  public async createScore(param: any): Promise<any> {
    ...
    const insertScore: any = await ModelScoring.query()
      .insert({
        id: generateId(),
        score: param.score,
    ...
    for (let index = 0; index < param.detail.length; index++) {
      ...
      const insertDetail: any = await ModelScoringDetail.query()
        .insert({
          id: generateId(),
  ...
  public async update(id: string, param: any): Promise<any> {
    ...
    await ModelScoringMaster.query()
      .update({
        name: param.name,
        description: param.description,
        start_date: param.start_date,
    ...
    //update question
    const section: any = await ModelScoringSection.query()
      .where("scoring_master_id", id)
      .count();
    if (section) {
      await ModelScoringSection.query().where("scoring_master_id", id).delete();
    }

    const question: any = await ModelScoringQuestion.query()
      .where("scoring_master_id", id)
      .count();
    if (question) {
      await ModelScoringQuestion.query()
        .where("scoring_master_id", id)
        .delete();
    }
    ...
    for (let index = 0; index < param.section.length; index++) {
      const element = param.section[index];
      let dataSection: any = await ModelScoringSection.query()
        .where("id", element.id)
        .first();
      if (dataSection) {
      ...
      for (
        ...
        let dataQuestion: any = await ModelScoringQuestion.query()
          .where("id", quest.id)
          .first();
        if (dataQuestion) {
          ...
          const options: any = await ModelScoringQuestionOption.query()
            .where("scoring_question_id", quest.id)
            .count();
          if (options) {
            await ModelScoringQuestionOption.query()
              .where("scoring_question_id", quest.id)
              .delete();
          }
          dataQuestion = await ModelScoringQuestion.query()
        ...
        for (let indexopt = 0; indexopt < quest.options.length; indexopt++) {
          ...
          let dataQuestionOpt: any = await ModelScoringQuestionOption.query()
            .where("id", option.id)
            .first();
          if (dataQuestionOpt) {
            dataQuestionOpt = await ModelScoringQuestionOption.query()
              ...
              .into(ModelScoringQuestionOption.tableName);
            if (!dataQuestionOpt)
              throw new HttpException(409, "Failed to save!");
          } else {
            dataQuestionOpt = await ModelScoringQuestionOption.query()
              .insert({
                id: generateId(),
    ...
    const updateData: any = await ModelScoringMaster.query()
      .select()
      .from(ModelScoringMaster.tableName)
      .withGraphFetched("[section.question]")
      .where("id", "=", id)
      .first();
    return updateData;
  }
  ...
  public async rescore(param: any): Promise<any> {
    ...
    const user: any = await ModelUser.query()
      ...
      .whereIn("email", param.user)
      .withGraphFetched("[role]");
    for (let index = 0; index < user.length; index++) {
      const element = user[index];
      //get okr task
      const okrTask: any = await ModelOkrTask.query().where(
        "mentee_id",
        element.id
      );
  ...
  public async rescoreAll(param: any): Promise<any> {
    //check user
    const leaderboard: any = await ModelLeaderboard.query()
